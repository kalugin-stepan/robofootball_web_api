...
public class Database {
    ...
    public async Task<uint> AddUser(MqttUser user) {
        ...
        var id = Convert.ToUInt32(await cmd.ExecuteScalarAsync());
    ...
    public async Task<bool> AddAcl(MqttAcl acl) {
        if (await GetAclByUsername(acl.username) != null) return false;

        var cmd = connection.CreateCommand();

        var access = (Int16)acl.access;

        cmd.CommandText = $"insert into mqtt_acl (username, access, topic) values ('{acl.username}', {access}, '{acl.topic}');";
